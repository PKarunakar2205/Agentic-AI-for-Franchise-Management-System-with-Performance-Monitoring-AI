...
export default function OutletPerformanceAgent() {
          ...
          {(sidebarOpen || true) && (
            ...
            >
              <div className="flex items-center gap-2 px-6 h-16 border-b border-slate-200/70 dark:border-slate-800">
                ...
                <span className="font-semibold tracking-tight text-slate-900 dark:text-white">FranchiseOS</span>
              ...
              <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                {sidebarItems.map((item) => (
                  ...
                  >
                    <item.icon size={17} className={item.active ? "text-white" : "text-slate-400 group-hover:text-blue-500"} />
                    <span className="font-medium truncate">{item.label}</span>
                    {item.active && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-white animate-pulse" />}
                  </button>
                ))}
              </nav>
              ...
              <div className="p-4 border-t border-slate-200/70 dark:border-slate-800">
                <div className="rounded-xl bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-500/10 dark:to-purple-500/10 border border-blue-100 dark:border-blue-500/20 p-3">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Monitoring 10 outlets in real time</p>
                </div>
              </div>
            </motion.aside>
          )}
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <header className="sticky top-0 z-20 h-16 border-b border-slate-200/70 dark:border-slate-800 bg-white/70 dark:bg-slate-950/70 backdrop-blur-xl flex items-center gap-4 px-4 lg:px-8">
            ...
            <div className="ml-auto flex items-center gap-2 lg:gap-3">
              ...
              <button className="relative p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors">
                <Bell size={18} />
                <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-rose-500 ring-2 ring-white dark:ring-slate-950" />
              </button>

              <button onClick={() => setDark(d => !d)} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors">
